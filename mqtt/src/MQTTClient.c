...
thread_return_type WINAPI MQTTClient_run(void* n)
{
	...
	while (!tostop)
	{
		...
		else
		{
			...
			{
				rc = TLSSocket_connect(&m->c->net.h, m->c->net.socket, m->c->cfg);
				if (rc == MQTTCLIENT_SUCCESS || rc == SSL_FATAL)
				{
					m->rc = rc;
					Log(TRACE_MIN, -1, "Posting connect semaphore for TLS client %s rc %d", m->c->clientID, m->rc);
					Thread_post_sem(m->connect_sem);
				}
			}
#endif
...
int MQTTClient_connectURI(MQTTClient handle, MQTTClient_connectOptions* options, const char* serverURI)
{
	...
	if (options->struct_version != 0 && options->ssl)
	{
		m->c->cfg = malloc(sizeof(tls_init_config_t));
...
#endif
#if defined(TLSSOCKET)
	if ((*sock = TLSSocket_getPendingRead()) == -1)
	{
#endif
		...
		*rc = TCPSOCKET_COMPLETE;
		while (1)
		{
			...
			if (sock == m->c->net.socket)
			{
				...
				{
					*rc = TLSSocket_connect(&m->c->net.h, sock, m->c->cfg);
					if ((*rc == SSL_FATAL) || (*rc == MQTTCLIENT_SUCCESS)) /* rc == 1 means SSL connect has finished and succeeded */
					{
						break;
					}
				}
