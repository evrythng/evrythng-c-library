...
#else
	/** The buffer using PEM format containing the public digital certificates trusted by the client. */
	char* trustStore;
	/** size of trustStore buffer*/
	int trustStore_size;

	/** The buffer using PEM format containing the public certificate chain of the client. It may also include
	* the client's private key. 
	*/
	char* keyStore;
	/** size of keyStore buffer*/
	int keyStore_size;
	
	/** If not included in the sslKeyStore, this setting points to the buffer using PEM format containing
	* the client's private key.
	*/
	char* privateKey;
	/** size of privateKey buffer*/
	int privateKey_size;
	/** The password to load the client's privateKey if encrypted. */
	const char* privateKeyPassword;
#endif   
...
#else
#define MQTTClient_SSLOptions_initializer { {'M', 'Q', 'T', 'S'}, 0, NULL, 0, NULL, 0, NULL, 0, NULL, NULL, 1 }
#endif
