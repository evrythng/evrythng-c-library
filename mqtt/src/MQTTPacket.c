...
#elif defined(TLSSOCKET)
	*error = (net->h) ? TLSSocket_getch(net->h, net->socket, &header.byte) : Socket_getch(net->socket, &header.byte); 
#else
...
#elif defined(TLSSOCKET)
	data = (net->h) ? TLSSocket_getdata(net->h, net->socket, remaining_length, &actual_len) : 
						Socket_getdata(net->socket, remaining_length, &actual_len);
#else
...
int MQTTPacket_send(networkHandles* net, Header header, char* buffer, size_t buflen, int free)
{
	...
	if (net->h)
		rc = TLSSocket_putdatas(net->h, net->socket, buf, buf0len, 1, &buffer, &buflen, &free);
	else
#endif
...
#if defined(TLSSOCKET)
	if (net->h)
		rc = TLSSocket_putdatas(net->h, net->socket, buf, buf0len, count, buffers, buflens, frees);
	else
#endif
...
#elif defined(TLSSOCKET)
		rc = (net->h) ? TLSSocket_getch(net->h, net->socket, &c) : Socket_getch(net->socket, &c);
#else
