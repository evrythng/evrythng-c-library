...
#elif defined(CONFIG_OS_FREERTOS)
	if ((rc = Thread_lock_mutex(*mutex)) == 0)
#else
...
#elif defined(CONFIG_OS_FREERTOS)
	if ((rc = Thread_unlock_mutex(*mutex)) == 0)
#else
...
#elif defined(CONFIG_OS_FREERTOS)
	return (unsigned long)Thread_getid();
#else
...
#else
		if ((rc = SSL_CTX_use_certificate_chain_buffer(net->ctx, opts->keyStore, opts->keyStore_size)) != 1)
#endif
		{
...
#else
		rc1 = SSL_CTX_use_PrivateKey_buffer(net->ctx, opts->privateKey, opts->privateKey_size, SSL_FILETYPE_PEM);
#endif
...
#else
		if ((rc = SSL_CTX_load_verify_buffer(net->ctx, opts->trustStore, opts->trustStore_size, SSL_FILETYPE_PEM)) != 1)
#endif
		{
