...
/**
 * Set a socket blocking, OS independently
 * @param sock the socket to set blocking
 * @return TCP call error code
 */
int Socket_setblocking(int sock)
{
	int rc;
#if defined(WIN32) || defined(WIN64)
	u_long flag = 0L;
...
int Socket_writev(int socket, iobuf* iovecs, int count, unsigned long* bytes)
{
	...
	*bytes = 0L;
    ...
    for(i = 0; i < count; ++i)
		tot += iovecs[i].iov_len;
    buf = malloc(tot);
    p = buf;
    for (i = 0; i < count; ++i)
	{
		memcpy (p, iovecs[i].iov_base, iovecs[i].iov_len);
		p += iovecs[i].iov_len;
    }
    rc = write (socket, buf, tot);
    free (buf);
	if (rc == SOCKET_ERROR)
	{
...
int Socket_putdatas(int socket, char* buf0, size_t buf0len, int count, char** buffers, size_t* buflens, int* frees)
{
	...
	if ((rc = Socket_writev(socket, iovecs, count+1, &bytes)) != SOCKET_ERROR)
	{
		...
		else
		{
			...
			SocketBuffer_pendingWrite(socket, 0, count+1, iovecs, frees1, total, bytes);
#else
